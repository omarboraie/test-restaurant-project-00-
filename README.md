...
------------------------------------------------
https://github.com/omarboraie/mocha-testing/tree/main
- Introduction to Mocha & Chai Testing (Core Imports: describe, it, expect)
- Structuring Unit Test Suites (Using describe blocks and individual it test cases)
- Testing Basic Functionality (Validating add(6, 3) returns 9)
- Data-Driven Testing with Test Cases (Using an array and loop to test multiple toCentimeter scenarios)
-  Introduction to API Unit Testing (Testing a User class that interacts with Axios)
- Testing Asynchronous Functions (Using Promises and the Mocha done callback)
- Stubbing External API Requests (Using Sinon to replace axios.get with a controlled response)
- Testing Successful API Responses (Verifying the returned user ID is a number and equals 1234)
- Verifying External API Calls (Checking Axios is called exactly once)
